...
def render_resultado(data):
    ...
    link = f"https://dev.azure.com/qualiit/Quali%20IT%20-%20Inovação%20e%20Tecnologia/_boards/board/t/Quali%20IT%20!%20Gestao%20de%20Projeto/Features?workitem={wi_id}"
    ...
    return html.Div([
        ...
        html.Div(f"👤 Atribuído a: {assigned_to} | 📅 Data Status Report: {created_date}"),

        html.Hr(),

        html.H5("📝 Status Report", className="text-primary mb-3"),
        _render_status_report(fields),

        html.Hr(),

        html.H5("📋 Informações Gerais do Projeto"),
        _render_custom_fields(fields)
    ])

def _render_status_report(fields):
    campos = {}

    for k, v in fields.items():
        if any(chave in k.lower() for chave in CAMPOS_STATUS_REPORT):
            chave = normalize_field_key(k.split('.')[-1])
            label = RENOMEAR_CAMPOS.get(chave, k.split('.')[-1])
            valor = clean_html(str(v))
            campos[label] = valor

    metade = len(campos) // 2 + len(campos) % 2
    itens = list(campos.items())

    def render_linha(label, valor):
        linhas = valor.split("\n")
        return html.P([
            html.Strong(f"{label}: "),
            html.Span(linhas[0]),
    ...
    col1 = html.Div([render_linha(k, v) for k, v in itens[:metade]])
    col2 = html.Div([render_linha(k, v) for k, v in itens[metade:]])
